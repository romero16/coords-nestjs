...
export class AuthService {
    constructor(
        @InjectRepository(AuthEntity) private auth: Repository<AuthEntity>,
    ...
    public async login( {email, password}:AuthDto){
            ...
            if (resp){
                // if (resp.status === "Activo"){
                    ...
                    //     const token = await this.jwtService.signAsync({id:resp.id,name:resp.name, email:resp.email, role:resp.role, status:resp.status});
                    //     return {
